...
function getFlight_data(icao24, callsign, tail, typecode) {
  ...
  let aircraft_id = document.getElementById("aircraft_id");
...
// draw all planes on the map
function drawPlanes(data) {
  let planeCountEl = document.getElementById("plane_count");
  planeCountEl.innerHTML = data.length;
...
function getPlanes(und = "", history = 0, icao24 = "", callsign = "") {
  url = "planes.geojson";
  const searchParams = new URLSearchParams({ history, und, icao24, callsign });
  url = url + "?" + searchParams;
  $.getJSON(url, function (data) { });
}
...
function getTrajectory(icao24) {
  ...
  $.getJSON(url, function (data) {
    traj.clearLayers();
    console.log('trajectory layer cleard')

    // L.geoJson(data, { color: "black" }).addTo(traj);
    console.dir(data);
    let plots = data.map(({ latitude, longitude }) => { return [latitude, longitude]; });
    L
      .polyline(plots, { color: 'black', weight: 1, smoothFactor: 2 })
      .addTo(traj); console.log('new trajectory rendered, icao24: ', icao24);
  });
}
