...
var traj = L.layerGroup(); // trajectory layer group

var map = L.map("map", { layers: [cat_mod, cat_sev, sigmets, aireps, turbulences, planes], }).setView([48, 5], 5);
...
channel.on("new-data", (data) => { // array
  window.planeData = data;
  console.dir(`plane count: ${data.length}`);
  if (data.length > 0) {
    drawPlanes(data);
  }
});
