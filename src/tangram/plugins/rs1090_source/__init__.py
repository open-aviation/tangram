...
async def icao24_track(url, identifier):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code not in [200]:
            logging.error('fail to get 5-minutes `track` from %s for %s, status: %s', url, identifier, resp.status_code)
            return None
    return resp.json()
...
def main():
    ...
    rs1090_data = Rs1090Data(args.base_url, 'http://127.0.0.1:18000/admin/publish')
    rs1090_data.all()
...
class PublishRunner:
    ...
    async def states(self):
        return {'running': self.running, 'counter': self.counter, 'task': self.task.done() if self.task else None}
