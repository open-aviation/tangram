...
pub async fn warp_on_connected(ws: WebSocket, state: Arc<State>) {
    let conn_id = nanoid::nanoid!(8).to_string(); // Generated by the server, used internally
    state.ctl.lock().await.conn_add_tx(conn_id.clone()).await;
    info!("on_connected, new connection: {}", conn_id);

    let (mut ws_tx, mut ws_rx) = ws.split();

    // conn rx => ws tx
    // Send messages to the client via websocket
    ...
    // This is connection end, not agent end
